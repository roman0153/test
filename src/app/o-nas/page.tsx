...
export default function AboutPage() {
  return (
    <>
      ...
      <section className="container about-story section-space"><div><LeafMark className="standalone-leaf" /><p className="eyebrow">Naša filozofia</p><h2>Nie iba pekná záhrada.<br />Váš kúsok sveta.</h2></div><div><p className="large-copy">Záhrada by mala byť miestom, kam sa chcete vracať. Kde môžete spomaliť, nadýchnuť sa a byť jednoducho doma.</p><p className="body-copy">V Eden Gardens vnímame exteriér ako pokračovanie domova. Prepájame záhradnú architektúru s každodenným životom — od prvého ranného svetla na terase až po posledný večerný pohľad z okna.</p><p className="body-copy">Našou ambíciou je tvoriť nadčasové priestory. Také, ktoré rešpektujú miesto, prirodzene starnú a každým rokom získavajú na charaktere.</p><TextLink href="/sluzby">Spoznajte náš prístup</TextLink></div></section>
      <section className="values-section section-space"><div className="container"><p className="eyebrow">Čomu veríme</p><div className="values-grid">{[
      ...
      ].map(([number, title, text]) => <div key={number}><span className="service-number">{number}</span><h3>{title}</h3><p className="body-copy">{text}</p></div>)}</div></div></section>
      <ContactBanner />
    </>
  );
}